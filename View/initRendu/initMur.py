...
def initMur(canvas):
    ySize = int(config().ReturnValue("HauteurMur"))
    MurColor = list(map(float, config().ReturnValue('MurColor').split(",")))
    
    #VAO
    canvas.VAOMur = glGenVertexArrays(1)
    glBindVertexArray(canvas.VAOMur)
    
    vertex = arrayVertCube(pos1=(0,0,0), pos2=(1, ySize, 1), color=[*MurColor])
    canvas.indexMur = arrayIndexCube(0)
    
    indice_buffer = glGenBuffers(1)
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indice_buffer)
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(canvas.indexMur)*4, canvas.indexMur, GL_STATIC_DRAW)
    ...
    loc_positions = glGetAttribLocation(canvas.shader_Wall, 'positions')
    ...
    loc_colors = glGetAttribLocation(canvas.shader_Wall, 'inObjectColor')
    ...
    glBufferData(GL_ARRAY_BUFFER, len(canvas.Mur)*4, canvas.Mur, GL_STATIC_DRAW)
    
    loc_offset = glGetAttribLocation(canvas.shader_Wall, 'Offset')
    ...
    canvas.loc_mvp = glGetUniformLocation(canvas.shader_Wall, "mvp")
    
    glBindVertexArray(0)
    
    print("Nombres de cube affichés en même temps:", int(len(canvas.Mur)/2)+int(len(canvas.Marge)/2))
