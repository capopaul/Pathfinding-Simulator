...
class AlgoAV3(threading.Thread):
    ...
    def _nextTourAction(self):
        ...
        self.CurrentSell = PlusPetitFOpenList(self)
    ...
    def _addNewValuesToOpenList(self):
        ...
        for edit in _sellToAdd:
            _g = self._G(self.CloseList, edit, self.CurrentSell)
            _h = self._H( edit)
            _f = _g + _h
            _parentX = self.CurrentSell[0]
            _parentZ = self.CurrentSell[1]
            self.OpenList[tuple(edit)] = [ _g, _h, _f, _parentX, _parentZ]
            
        for sell in _sellInOpenList:
            _g = self._G(self.CloseList, sell, self.CurrentSell)
            if(_g <  self.OpenList[sell[0], sell[1],0]):
                _h = self._H(sell)
                _f = _g + _h
                _parentX = self.CurrentSell[0]
                _parentZ = self.CurrentSell[1]
    ...
    def _caseVideAutourCanvasMap(self):
        ...
        for edit in _cote:
            ...
            (_z) >=0 and (_z) < (len(self.canvas.tableauMap))):
                ...
                if self.canvas.tableauMap[_z][_x] != 1:
